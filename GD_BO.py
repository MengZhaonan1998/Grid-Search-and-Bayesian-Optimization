...
def bayesian_opt(train,label, n_init=1, maxiter=10, opt="GD", verbose=0,acq_funct=1):
    ...
    def gaussian_kernel(x1,x2,gamma=5e-1):      
    ...
    acq = second_acq  
    if opt == "GD":
        minimum = gradient_descent  # classical gradient descent method
        if acq_funct == 1:          #setting acquisition function choice
            acq = first_acq
            np.random.seed(1)
        elif acq_funct == 2:
            acq = second_acq
            np.random.seed(4)        # randomly sample n_init initial points for hyperparameter (hlist): C,gamma
        gamma = 5e-1                 # kernel size
        a_interval = (0,-10)
        b_interval = (9,  0)
    else:
...
def run_gd_bo(maxiter,verbose,acq_choice):
    data = load_data(dir_path+'\heart.csv')                 # load .csv
    train,label = data_preprocessing(data)                  # derive training data and label
    acq_map, min_path, cvg_hist, hlist, best_h, y_best = bayesian_opt(train,label,maxiter=maxiter,opt="GD",verbose=verbose,acq_funct=acq_choice)    # bayesian optimization
    return acq_map, min_path, cvg_hist, hlist, best_h, y_best